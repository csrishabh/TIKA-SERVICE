...
public class DocumentProcesser {
	...
	public void getTextAndPdfFromImage(File file) {
		...
		try {
			...
			String output_fname = path + File.separator + file.getName().substring(0, file.getName().lastIndexOf('.'));
	...
	public File getProcessedFile(File file) throws IOException {
		...
		File Processedfile = new File(output_fname + ".tiff");
	...
	/*public static void main(String[] args) {
		File f = new File("C:\\Users\\rishabh.jain1\\Downloads\\LargeExcelFiles\\$100 Home Depot Gift Card.PDF");
		try {
			 String hocr = new DocumentProcesser().getHocr(f);
